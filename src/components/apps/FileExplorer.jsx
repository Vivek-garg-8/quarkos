      ...
      {Object.entries(data).map(([name, item]) => {
        ...
        return (
          <li key={name} className="my-1">
            <div
              className="flex items-center gap-2 p-1 rounded-md hover:bg-zinc-700 cursor-pointer"
              onClick={() => {
                if (isFolder) {
                  toggleExpand(name);
                }
                onSelect(item, currentItemPath, name);
              }}
            >
              {isFolder && (
                <span className="w-4">{expanded[name] ? "▼" : "►"}</span>
              )}
              <div className="w-5 h-5">
                {item.type === "drive" ? (
                  <DriveIcon />
                ) : isFolder ? (
                  <FolderIcon isOpen={expanded[name]} />
                ) : (
                  <FileIcon />
                )}
              </div>
            ...
            {isFolder && expanded[name] && (
              <div className="pl-6 border-l border-zinc-600">
                <TreeView
                  data={item.children}
                  onSelect={onSelect}
...
const FileExplorer = () => {
  const { fs, createItem, deleteItem, renameItem , updateFileContent} = useFileSystem();
  const [currentPath, setCurrentPath] = useState(["C:"]);
  const [selectedInView, setSelectedInView] = useState(null); // This state now controls selection

  const currentDirectory = getObjectFromPath(fs, currentPath);

  const handleNewFolder = () => {
    const folderName = prompt("Enter folder name:", "New Folder");
    if (folderName) createItem(currentPath, folderName, "folder");
  };
  const handleNewFile = () => {
    const fileName = prompt("Enter file name:", "New File.txt");
    if (fileName) createItem(currentPath, fileName, "file");
  };
  const handleDelete = () => {
    if (!selectedInView) return alert("No item selected.");
    if (
      window.confirm(`Are you sure you want to delete "${selectedInView}"?`)
    ) {
      deleteItem(currentPath, selectedInView);
  ...
  const handleRename = () => {
    ...
    const newName = prompt(
      `Enter new name for "${selectedInView}":`,
      selectedInView
    );
    if (newName && newName !== selectedInView) {
      renameItem(currentPath, selectedInView, newName);
      setSelectedInView(newName);
    }
  };
  ...
  const menuBarItems = [
    ...
    },
    {
      label: "Edit",
      actions: [
        { label: "Rename", onClick: handleRename, disabled: !selectedInView },
        { label: "Delete", onClick: handleDelete, disabled: !selectedInView },
      ],
    },
  ];
